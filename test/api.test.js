...
describe("API pow", () => {
  test("Si uno de los parámetros no es un número, el endpoint debe devolver el mensaje de error correspondiente, junto a un status 400", async () => {
    const app = await api.build();
    ...
    expect(res.body).toEqual({
      error: "La base ingresada por parametro no es un numero",
    });
  });
});
...
describe("API getHistory", () => {
  test("Debería devolver todo el historial", async () => {
    ...
    const res = await request(app).get("/api/v1/getHistory").expect(200);
...
describe("API deleteHistory", () => {
  test("Debería eliminar todo el historial", async () => {
    ...
    const res1 = await request(app).delete("/api/v1/deleteHistory").expect(200);
